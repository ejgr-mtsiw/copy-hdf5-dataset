...
int read_attributes(const hid_t dataset_id, dataset_t *dataset) {
	...
	// Get attributes
	read_attribute(dataset_id, N_CLASSES_ATTRIBUTE, H5T_NATIVE_INT, &n_classes);
	...
	read_attribute(dataset_id, N_OBSERVATIONS_ATTRIBUTE, H5T_NATIVE_INT,
			&n_observations);
	...
	read_attribute(dataset_id, N_ATTRIBUTES_ATTRIBUTE, H5T_NATIVE_INT,
			&n_attributes);
	...
	unsigned int n_jnsqs = 0;
	// Number of jnsqs in the dataset
	read_attribute(dataset_id, N_JNSQS_ATTRIBUTE, H5T_NATIVE_INT, &n_jnsqs);

	// Store data
	dataset->n_attributes = n_attributes;
	dataset->n_bits_for_class = (unsigned int) ceil(log2(n_classes));
	dataset->n_classes = n_classes;
	dataset->n_jnsqs = n_jnsqs;
